...
export type ProjectListProps = {
  selectedId?: string;
  onSelect?: (id: Project['id']) => void; // TODO 05: (TS) onSelect 의 시그니처를 더 안전하게 개선해보세요. string 보다 구체적인 타입 사용 고려
};
  ...
  return (
    <div>
      ...
      <ul>
        {projects.map((p) => (
          <li key={p.id}>
            <button style={{ fontWeight: selectedId === p.id ? 'bold' : 'normal' }} onClick={() => onSelect?.(p.id)}>
              {p.name}
            </button>
          </li>
        ))}
